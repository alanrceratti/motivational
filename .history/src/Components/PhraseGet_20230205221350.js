...
function PhraseGet() {
	...
	async function fetchData() {
		...
		const [url, setUrl] = useState(`https://example.com/api/${category}/${phraseId}`);

		// const shareURL = `https://motivational-api-2kzjz.ondigitalocean.app/api/categories/${categoryId}/phrases/?filter_by_id=${number}`;
		// window.location.href = shareURL;
		// function shareLink(categoryId, number) {
		// 	window.location = `https://motivational-api-2kzjz.ondigitalocean.app/api/categories/${categoryId}/phrases/?filter_by_id=${number}`;
		// }

		// async function sharePhrase() {
		// 	var shareURL =
		// 		"http://localhost:3000/?text=" +
		// 		encodeURIComponent(sharePhrase);
		...
		<section className={styles.mainContainer}>
			{idLists.length <= 0 ? (
				...
				<>
					...
					/>
					{/* <button onclick="navigator.clipboard.writeText('sharePhrase');">
						Share
					</button>

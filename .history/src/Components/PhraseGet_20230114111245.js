...
function PhraseGet() {
    ...
    let selectedIdList = idLists[e.target.value];
    if (!selectedIdList) {
        console.log("Choose another category");
        return;
    }

    let previousIdList = JSON.parse(localStorage.getItem(`previousid${e.target.value}`)) || [];
    let filteredIdList = selectedIdList.filter((number) => !previousIdList.includes(number));
    if (filteredIdList.length > 0) {
        const randomIndex = Math.floor(Math.random() * filteredIdList.length);
        const randomNumber = filteredIdList[randomIndex];
        setNumber(randomNumber);
        previousIdList.push(randomNumber);
        localStorage.setItem(`previousid${e.target.value}`, JSON.stringify(previousIdList));
    } else {
        localStorage.removeItem(`previousid${e.target.value}`);

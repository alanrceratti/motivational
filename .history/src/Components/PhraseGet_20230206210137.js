...
function PhraseGet() {
	...
	const UrlPhraseId = searchParams.get("phraseId");

	useEffect(() => {
		fetch(`https://your-api.com/category/${categoryId}/phrase/${number}`)
	...
	console.log("hueheueuh");

	////////////////////////////////////////////////////////

	useEffect(() => {
		if (UrlCategoryId && UrlPhraseId) {
			setCategoryId(Number(UrlCategoryId));
			setNumber(Number(UrlPhraseId));
		}
	}, []);
	const [phraseId, setPhraseId] = useState(UrlPhraseId || 8);

	useEffect(() => {
		const newPhraseId = UrlPhraseId || 8;
		setPhraseId(newPhraseId);
	}, [UrlPhraseId]);

	useEffect(() => {
		fetch(`https://your-api.com/category/${categoryId}/phrase/${phraseId}`)
			.then((response) => response.json())
			.then((data) => {
				setCategoryId(data.categoryId);
				setPhraseId(data.phraseId);
			});
	}, [categoryId, phraseId]);

	// Function to handle selecting a new phrase
	const handleChangePhrase = (newPhraseId) => {
		setPhraseId(newPhraseId);
		const currentUrl = window.location.href;
		const newUrl =
			currentUrl.split("?")[0] +
			`?categoryId=${categoryId}&phraseId=${newPhraseId}`;
	...
	const handleNextButton = () => {
		...
		history.push(`/phrase-get?categoryId=${categoryId}&phraseId=${number}`);
	};
	...
	// 		const categoryIdFromLocalStorage =
	...
	async function fetchData() {
		...
		window.history.pushState(
			{},
			"",
			`/category/${categoryId}/phrase/${number}`
		);
	}
	return (

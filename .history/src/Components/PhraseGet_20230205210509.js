...
function PhraseGet() {
	...
	function handleChangeNext(categoryId) {
		...
		if (filteredIdList.length > 0) {
			...
			window.history.pushState(
				{},
				"",
				`/category/${categoryId}/phrase/${randomNumber}`
			);
			fetchData();
		} else {
	...
	const API_URL = `https://motivational-api-2kzjz.ondigitalocean.app/api/categories/${categoryId}/phrases/?filter_by_id=${number}`;

	useEffect(() => {
		fetch(API_URL)
			.then((response) => response.json())
			.then((data) => window.history.pushState(API_URL);
	}, [API_URL]);

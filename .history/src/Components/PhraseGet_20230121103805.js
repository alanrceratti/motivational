...
function PhraseGet() {
	...
	function handleChangeNext(categoryId) {
		...
		const currentCategory =	localStorage.setItem(`currentCat${selectedCategoryId}`)

		const currentCat = categoryId;
		localStorage.setItem(`currentCategory`, JSON.stringify(currentCat));

		const PreviousCategory = JSON.parse(localStorage.getItem(currentCat));

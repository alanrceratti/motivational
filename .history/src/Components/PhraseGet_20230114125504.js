...
function PhraseGet() {
	...
	async function fetchCount() {
		...
		const IDs = json;
		console.log(IDs, idLists[1]);

		IDs.forEach((category) => {
			const categoryId = category.id;
			const categoryPhrases = category.phrases.id;
			const ids = categoryPhrases.map((phrase) => phrase.id);
			idLists[categoryId].push(categoryPhrases);
		});
		console.log(categoryId, categoryPhrases[1]);
	}
	const idLists = {
		1: [],
		2: [],

...
function PhraseGet() {
	...
	function handleChangeCategory(e) {
		...
		if (e.target.value === "1") {
			...
			let previousid1 = JSON.parse(localStorage.getItem("previousid1")) || [];
			// filter the id1 array to exclude the previousid1 numbers
			let filteredId1 = id1.filter(number => !previousid1.includes(number));
			// if there are still numbers available in filteredId1 array
			if (filteredId1.length > 0) {
			  const randomIndex = Math.floor(Math.random() * filteredId1.length);
			  randomNumber = filteredId1[randomIndex];
			  setNumber(randomNumber.toString());
			  id1.splice(id1.indexOf(randomNumber), 1);
			  ...
			  localStorage.setItem("previousid1", JSON.stringify(previousid1));
		} else if (e.target.value === "2") {
				let randomNumber;
				// retrieve previousid1 array from local storage
				let previousid2 = JSON.parse(localStorage.getItem("previousid2")) || [];
				// filter the id1 array to exclude the previousid2 numbers
				let filteredId1 = id1.filter(number => !previousid2.includes(number));
				// if there are still numbers available in filteredId1 array
				if (filteredId1.length > 0) {
				  const randomIndex = Math.floor(Math.random() * filteredId1.length);
				  randomNumber = filteredId1[randomIndex];
				  ...
				  localStorage.setItem("previousid2", JSON.stringify(previousid2));
				} else {
			console.log("All numbers have been used, please refresh the page!");
		}
	}
}

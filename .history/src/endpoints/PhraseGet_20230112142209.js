...
function PhraseGet() {
	...
	function handleChangeCategory(e) {
		...
		async function fetchCategories() {
			const response = await fetch(`http://127.0.0.1:200/api/categories/${e.target.value}/`);
			const json = await response.json();
			const arr = json

		if (e.target.value === "1") {
			const randomIndex = Math.floor(Math.random() * arr.id);
			const randomNumber = id1[randomIndex];
			console.log("numbero random", randomNumber);

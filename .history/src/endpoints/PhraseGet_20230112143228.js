	...
	function handleChangeCategory(e) {
		...
		async function fetchCategories() {
			const response = await fetch(
				`http://127.0.0.1:200/api/categories/1/`
			);
			const json = await response.json();
			const arr = json;
			console.log(arr.phrases);

			if (e.target.value === "1") {
				const randomIndex = Math.floor(Math.random() * arr.phrases.id);
				setNumber(arr.phrases.id);
				console.log(randomIndex);
			} else if (e.target.value === "2") {
				const randomIndex = Math.floor(Math.random() * arr.phrases.id);
				setNumber(arr.phrases.id);
				console.log(arr.phrases.id);
			} else {
				console.log("nao deu");
			}
